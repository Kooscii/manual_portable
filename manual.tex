...
\begin{document}
			...
			\subsubsection{Mirror}\label{sec:mirror}
			 \begin{tabularx}{\textwidth}{ cX }
				\raisebox{-0.9\height}{\includegraphics[width=0.33\textwidth]{intro1-1-3.jpg}} &
			 	...
			 	about this will be introduce in section \ref{sec:linear-stage}. If you have to adjust the mirror, please see the appendix \ref{sec:mirror_adjustment}.
			 \end{tabularx}
			...
			\subsubsection{Linear Stage}\label{sec:linear-stage}
			 \begin{tabularx}{\textwidth}{ cX }
				\raisebox{-0.9\height}{\includegraphics[width=0.33\textwidth]{intro1-1-4.jpg}} &
			 	...
			 	lower the stage (as well as the sample holder) and counterclockwise upper the stage. The stage is fixed on 
			 	a bign base and the mirror mentioned above is fixed on it too. So Let's define the \textit{stage assembly} 
	...
	\section{Appendix}\label{sec:appendix}
		\subsection{Convex lens adjustment}\label{sec:convex_lens_adjustment}
		 \begin{itemize}
			\begin{tabularx}{\textwidth}{ Xc }
		 	\item The knob X and knob Z can adjust the position of the convex lens in X, Z dimension as well as the rotation in 
		 	X, Z axis. And the screw Y can adjust the lens' position in Y dimension. You should make sure that the lens is 
		 	central and vertical with the main optical axis, which means it should be parallel and aligned with the imaging 
		 	lens of the projector. The distance between convex and imaging lens will make sense to the imaging distance. 
		...
		\begin{itemize}
			\begin{tabularx}{\textwidth}{ Xc }
		 	\item The knob U, V, W can adjust the position and the rotation in three dimensions. First you should adjust the 
		 	mirror coarsely and let the image projected on the sample holder. Not being in the center is OK, you can make 
		 	fine adjustment in the next step. 
		 	&\raisebox{-\height}{\includegraphics[width=0.33\textwidth]{mirror_adjustr.jpg}}\\
		 	\\
		 	\end{tabularx}

		 	\begin{tabularx}{\textwidth}{ Xc }
		 	\item Then you need to do the fine adjustment. The knob A can rotate the mirror and let the image move in Y axis. 
		 	The knob B can rotate the mirror too and let the image move in X axis. So you just need to rotate the knob and 
		 	make the image pojected in the center of the sample holder. 
